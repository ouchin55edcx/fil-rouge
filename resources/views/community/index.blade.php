                        ...
                        <div id="popup2" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
                            <div class="bg-white p-4 rounded-lg w-[50vw]">
                                ...
                                <form action="{{route('asks.store')}}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <input class="title bg-gray-100 border border-gray-300 p-2 mb-2 outline-none w-full" spellcheck="false" placeholder="statrt your question with 'What', 'How', 'Why' ect ..." type="text" name="content">
            ...
            @foreach($communityItems as $item)
                @if($item->type === 'post')
                    <div class="border max-w-screen-md bg-white mt-6 p-4">
                        <!-- Your post HTML content here -->
                        <div class="flex items-center justify-between">
                            <!-- Post content -->
                            <div class="whitespace-pre-wrap mt-7 font-bold"># {{ $item->title }}</div>
                            <div class="whitespace-pre-wrap mt-7 font-medium">{{ $item->content }}</div>
                            <!-- Other post details and interactions -->
                        </div>
                    </div>
                @elseif($item->type === 'ask')
                    <div class="bg-white mt-4 cursor-pointer transition duration-350 ease-in-out pb-4 border-l border-r">
                        <!-- Your ask HTML content here -->
                        ...
                        <!-- Other ask details and interactions -->
                    </div>
                @endif
            @endforeach
...
{{--                                <b class="mb-2 capitalize">{{Auth::User()->username}}</b>--}}
{{--                                <time datetime="{{ $post->created_at->toAtomString() }}" class="text-gray-400 text-xs">{{ $post->created_at->format('d F \a\t h:i A') }}</time>--}}
{{--                            </div>--}}
{{--                        </div>--}}
